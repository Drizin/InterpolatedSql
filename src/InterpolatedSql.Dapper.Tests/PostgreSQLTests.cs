...
namespace InterpolatedSql.Dapper.Tests
{
    public class PostgreSQLTests
    {
        ...
        public void TearDown()
        {
            ...
            InterpolatedSql.Dapper.QueryBuilder.DefaultOptions.DatabaseParameterSymbol = "@";
            InterpolatedSql.Dapper.QueryBuilder.DefaultOptions.AutoGeneratedParameterPrefix = "p";
        }
        #endregion
        ...
        {
            var titles = SqlMapper.Query<TitleRecord>(cn, "SELECT * FROM film");
        ...
        public void TestParameters()
        {
            ...
            var titles = cn.QueryBuilder($"SELECT * FROM film WHERE title like {search}").Build().Query<TitleRecord>();
            Assert.That(titles.Any());
        }
        ...
        public void TestAutoGeneratedParameterPrefix()
        {
            ...
            InterpolatedSql.Dapper.QueryBuilder.DefaultOptions.DatabaseParameterSymbol = ":";
            InterpolatedSql.Dapper.QueryBuilder.DefaultOptions.AutoGeneratedParameterPrefix = "parm";

            string search = "%Dinosaur%";
            var cmd = cn.QueryBuilder($"SELECT * FROM film WHERE title like {search}");

            Assert.AreEqual("SELECT * FROM film WHERE title like :parm0", cmd.Build().Sql);
        }
