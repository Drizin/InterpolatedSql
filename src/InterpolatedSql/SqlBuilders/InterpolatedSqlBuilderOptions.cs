...
namespace InterpolatedSql.SqlBuilders
{
    ...
    public class InterpolatedSqlBuilderOptions
    {
        ...
        /// </summary>
        public Func<object?, string> FormatArgumentForDisplay { get; set; }
        #endregion
        ...
        public InterpolatedSqlBuilderOptions()
        {
            ...
            FormatArgumentForDisplay = (value) => 
            {
                if (value != null && value is InterpolatedSqlParameter)
                    value = ((InterpolatedSqlParameter)value).Argument;
                ...
                if (value is IEnumerable<object> list)
                    return "[" + string.Join(", ", list.Select(item => FormatArgumentForDisplay!(item))) + "]";
                if (value is IEnumerable list2)
        ...
        /// </summary>
        public static InterpolatedSqlBuilderOptions Create()
        {
            return new InterpolatedSqlBuilderOptions()
            {
                ArgumentComparer = DefaultOptions.ArgumentComparer,
                AutoAdjustMultilineString = DefaultOptions.AutoAdjustMultilineString,
                AutoEscapeCurlyBraces = DefaultOptions.AutoEscapeCurlyBraces,
                AutoFixSingleQuotes = DefaultOptions.AutoFixSingleQuotes,
                AutoGeneratedParameterPrefix = DefaultOptions.AutoGeneratedParameterPrefix,
                AutoSpacingOptions = DefaultOptions.AutoSpacingOptions,
                CalculateAutoParameterName = DefaultOptions.CalculateAutoParameterName,
                DatabaseParameterSymbol = DefaultOptions.DatabaseParameterSymbol,
                FormatParameterName = DefaultOptions.FormatParameterName,
                FormatArgumentForDisplay = DefaultOptions.FormatArgumentForDisplay,
                ParameterArrayNameSuffix = DefaultOptions.ParameterArrayNameSuffix,
                Parser = DefaultOptions.Parser,
                PreserveArgumentFormatting = DefaultOptions.PreserveArgumentFormatting,
                ReuseIdenticalParameters = DefaultOptions.ReuseIdenticalParameters,
            };
        }
