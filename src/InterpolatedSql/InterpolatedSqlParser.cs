...
{
    #region IInterpolatedSqlParser
    /// <summary>
    /// Parses FormattableString into <see cref="InterpolatedSqlBuilder"/> using regex.
    /// </summary>
    public interface IInterpolatedSqlParser
    {
        /// <summary>
        /// Parses a FormattableString and Appends it to an existing <see cref="InterpolatedSqlBuilder"/>
        /// </summary>
        void ParseAppend(FormattableString value, InterpolatedSqlBuilderBase target);

        /// <summary>
        /// Parses a FormattableString and Inserts it at specified position into an existing <see cref="InterpolatedSqlBuilder"/>
        /// </summary>
        void ParseInsert(FormattableString value, InterpolatedSqlBuilderBase target, int index);
    ...
    /// <summary>
    /// Parses FormattableString into <see cref="InterpolatedSqlBuilder"/>
    /// </summary>
    public class InterpolatedSqlParser : IInterpolatedSqlParser
    {
        ...
        /// </summary>
        public virtual void ParseAppend(FormattableString value, InterpolatedSqlBuilderBase target)
        {
            ParseInsert(value, target, target.Length);
        }

        /// <summary>
        /// Parses a FormattableStrings and Inserts it into an existing InterpolatedSql
        /// </summary>
        public virtual void ParseInsert(FormattableString value, InterpolatedSqlBuilderBase target, int index)
        {
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                AppendLiteral(target, format, literalStart, literalEnd - literalStart); //TODO: should be InsertLiteral, passing startIndex=index. StartIndex should be updated accordingly. Same for AppendArgument
        ...
        protected virtual void AppendLiteral(InterpolatedSqlBuilderBase target, string value, int startIndex, int count)
        {
            target.AppendLiteral(value, startIndex, count);
        }

        /// <summary>Appends a Literal</summary>
        protected virtual void AppendLiteral(InterpolatedSqlBuilderBase target, string value)
        {
        ...
        /// </summary>
        protected virtual void ProcessArgument(InterpolatedSqlBuilderBase target, object? argument, int argumentAlignment = 0, string? argumentFormat = null)
        {

            // If we get a nested InterpolatedSqlBuilder, it's already parsed - we just merge to current one
            if (argument is IInterpolatedSql isbArg)
            {
            ...
            {
                var nestedStatement = new InterpolatedSqlBuilder();
        ...
        /// </summary>
        protected virtual void AppendArgument(InterpolatedSqlBuilderBase target, object? argument, int argumentAlignment, string? argumentFormat)
        {
